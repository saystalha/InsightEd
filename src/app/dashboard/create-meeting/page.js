...
export default function CreateMeetingPage() {
  ...
  if (step === 2) {
    return (
      <div className="min-h-screen hero-bg flex items-center justify-center p-4 relative overflow-hidden">
        <div className="absolute -top-24 -right-24 w-[450px] h-[450px] rounded-full orb-copper animate-float pointer-events-none opacity-50" />
        ...
        <div className="relative z-10 w-full max-w-[440px] animate-fadeUp">
          <div className="card-navy rounded-[24px] p-8 text-center border border-white/5 shadow-[0_24px_64px_rgba(10,14,22,0.5)]">
            ...
            <div className="w-14 h-14 rounded-full flex items-center justify-center mx-auto mb-5 transition-transform duration-500 scale-100 hover:scale-105"
              style={{ background: 'rgba(196,124,62,0.14)', border: '1.5px solid rgba(196,124,62,0.35)' }}>
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#c47c3e" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                <path d="M22 11.08V12a10 10 0 1 1-5.93-9.14" />
                <polyline points="22 4 12 14.01 9 11.01" />
              </svg>
            </div>

            <h2 id="meeting-created-title" className="text-xl font-black text-white tracking-tight mb-1">Classroom Initialized</h2>
            ...
            <div id="class-code-display" className="mb-6">
              <p className="text-[10px] font-bold uppercase tracking-widest text-white/40 mb-2.5">Access Code</p>
              <div className="flex items-center justify-center gap-1.5 mb-3.5">
                {generatedCode.split('').map((ch, i) => (
                  <div key={i} className="w-10 h-12 rounded-xl flex items-center justify-center text-xl font-mono font-black text-white bg-black/30 border border-white/10 shadow-inner">
                    {ch}
                  </div>
                ))}
              </div>
              ...
              <button 
                ...
                className="flex items-center gap-1.5 mx-auto text-xs font-bold transition-all text-copper-lt hover:brightness-110 active:scale-95 px-3 py-1 rounded-md bg-white/5 border border-white/5"
              >
            ...
            <div className="text-left rounded-xl p-4 mb-6 bg-black/20 border border-white/5">
              <p className="text-xs font-bold text-white mb-0.5 truncate">{form.title}</p>
              ...
              <div className="mt-2.5 pt-2 border-t border-white/5 flex justify-between text-[10px] font-bold text-white/40 uppercase tracking-wider">
                <span>Alert Condition:</span>
                <span className="text-copper-lt">&lt; {threshold}% Attention</span>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-3">
              ...
              <Link href="/dashboard" className="text-xs font-bold text-white/40 hover:text-white transition-colors mt-2">
                ← Return to Dashboard
              </Link>
            </div>
  ...
  return (
    <div className="min-h-screen dashboard-bg relative overflow-hidden text-snow flex flex-col justify-between">
      <div className="absolute -top-40 -right-40 w-[600px] h-[600px] rounded-full orb-copper animate-float pointer-events-none opacity-40" />
      ...
      <div className="max-w-xl w-full mx-auto px-6 py-10 flex-1 flex flex-col justify-center relative z-10">
        ...
        <div className="mb-6 animate-fadeUp">
          <h1 id="create-meeting-title" className="text-[1.65rem] font-black text-white tracking-tight mb-1.5">Start a New Class</h1>
        ...
        <form id="create-meeting-form" onSubmit={handleSubmit} className="flex flex-col gap-4 animate-fadeUp [animation-delay:80ms]">
          ...
          <div className="p-6 rounded-[22px] flex flex-col gap-4 card-navy border border-white/5 shadow-xl">
            
            {/* Input Element Container: Class Title */}
            <div className="flex flex-col gap-1.5">
              <label htmlFor="meeting-title" className="text-[11px] font-bold uppercase tracking-wider text-white/60">
                Class Title <span className="text-copper font-black">*</span>
              </label>
              <input 
                id="meeting-title" 
                type="text" 
                ...
                className="px-4 py-3 rounded-xl text-white text-xs bg-black/20 border border-white/10 focus:border-[#c47c3e] placeholder:text-white/20 outline-none transition-all" 
              />
            </div>

            {/* Input Element Container: Description */}
            <div className="flex flex-col gap-1.5">
              <label htmlFor="meeting-desc" className="text-[11px] font-bold uppercase tracking-wider text-white/60">
                Description <span className="text-white/20 font-medium lowercase">(optional)</span>
              </label>
              <textarea 
                id="meeting-desc" 
                rows={3} 
                ...
                className="px-4 py-3 rounded-xl text-white text-xs bg-black/20 border border-white/10 focus:border-[#c47c3e] placeholder:text-white/20 outline-none resize-none transition-all leading-relaxed" 
              />
            </div>
          </div>

          {/* Interactive Engagement Alert Threshold Panel */}
          <div className="p-6 rounded-[22px] flex flex-col gap-3.5 card-navy border border-white/5 shadow-xl">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-xs font-black text-white uppercase tracking-wider">Engagement Alert Threshold</h2>
              ...
              <span id="threshold-value" className="text-xl font-mono font-black text-copper min-w-[48px] text-right">{threshold}%</span>
            </div>
            ...
            <input 
              ...
              style={{ background: `linear-gradient(90deg, #c47c3e ${((threshold - 10) / 70) * 100}%, rgba(255,255,255,0.06) ${((threshold - 10) / 70) * 100}%)` }} 
            />
            
            <div className="flex justify-between text-[10px] font-bold tracking-wide text-white/30 uppercase px-0.5">
      ...
      <footer className="w-full flex justify-center py-4 border-t border-white/5 flex-shrink-0 bg-black/10 z-10">
        <span className="flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider text-white/30 px-3 py-1 rounded-full bg-white/5 border border-white/5">
