...
export default function FaqsSection() {
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none" style={{ background: 'radial-gradient(circle at 50% 50%, rgba(59, 130, 246,0.06) 0%, transparent 70%)' }} />
            ...
            return (
              <div
                ...
                className={`rounded-2xl transition-all duration-300 border ${
                  isOpen 
                    ? 'bg-[#FFFFFF] border-[rgba(59, 130, 246,0.30)] shadow-[0_12px_30px_rgba(10,14,22,0.25)]' 
                    : 'card-navy border-black/5 hover:border-black/10 hover:bg-[#EAECEB]/20'
                }`}
              >
                ...
                >
                  <span className={`text-[0.95rem] font-bold tracking-tight transition-colors duration-200 ${isOpen ? 'text-[#1D4ED8]' : 'text-snow group-hover:text-[#3B82F6]'}`}>
                    {faq.question}
                  </span>
                  ...
                  <div className={`w-6 h-6 rounded-lg flex items-center justify-center border transition-all duration-300 flex-shrink-0 ${
                    isOpen ? 'border-copper bg-copper text-snow rotate-180' : 'border-black/10 bg-black/5 text-mist group-hover:bg-[#3B82F6] group-hover:text-white group-hover:border-[#3B82F6]'
                  }`}>
                    {isOpen ? (
                ...
                >
                  <div className="px-6 pb-5 text-xs sm:text-sm text-mist/90 leading-relaxed text-justify border-t border-black/5 pt-3">
                    {faq.answer}
                  </div>
                </div>
